...
#fit KM curve
survfit.luke.1 <- survfit(surv.luke.1 ~ 1, data=data, conf.type="log-log", conf.int=0.90)
...
aft.luke.1 <- flexsurvreg(surv.luke.1 ~ age + male + donorage + donormale + cmv + donorcmv + waittime + disgroup + fab + mtx, data=data, dist="gengamma")

aft.luke.1

aft.luke.est <- round(exp(aft.luke.1$res[4:13, 1:3]), 3)
...
data$waittime_long <- ifelse(data$waittime >178, 1 , 0)

survdiff(surv.luke.1 ~ age_old, data=data, rho=0)          #p-value 0.1
survdiff(surv.luke.1 ~ donorage_old, data=data, rho=0)     #p-value 0.7
...
survdiff(surv.luke.1 ~ waittime_long, data=data, rho=0)    #        0.6
...
legend("bottomright", c("ALL", "AML low", "AML high"), col=c("blue", "red", "orange"), lwd=c(2,2,2), lty=c("solid", "solid", "dashed"))
...
legend("bottomright", c("FAB 4 or 5 & AML", "other"), col=c("blue", "red"), lwd=c(2,2), lty=c("solid", "solid"))
